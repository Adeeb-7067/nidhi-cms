...
import { resolveCompanyIdFromBody, getProjectAccess } from "../lib/company-access";
...
router.get("/projects/:id/bugs", requireAuth, async (req, res) => {
  const { bugsTable } = await import("@workspace/db/schema");
  const projectId = parseInt(req.params['id'] as string);
...
router.get("/projects/:id/history", requireAuth, async (req, res) => {
