...
import { getProjectAccess, clientVisibilityFilter } from "../lib/inventory-access";
import { encryptSecret, decryptSecret } from "../lib/inventory-crypto";
import { logInventoryActivity, notifyProjectMembers } from "../lib/inventory-helpers";
...
router.get("/projects/:projectId/inventory/summary", requireAuth, async (req, res) => {
...
router.get("/projects/:projectId/inventory/activities", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
...
// ─── Folders ───────────────────────────────────────────────────────────────
router.get("/projects/:projectId/inventory/folders", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId))) return;
...
router.post("/projects/:projectId/inventory/folders", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
...
// ─── Resources ─────────────────────────────────────────────────────────────
router.get("/projects/:projectId/inventory/resources", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  const access = await guard(req, res, projectId);
  if (!access) return;
...
router.post("/projects/:projectId/inventory/resources", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  const access = await guard(req, res, projectId);
  if (!access) return;
...
router.patch("/projects/:projectId/inventory/resources/:id", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId, true))) return;
...
router.delete("/projects/:projectId/inventory/resources/:id", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId, true))) return;
  const id = parseInt(req.params.id as string);
  const resource = await inventoryResourcesTable.findOneAndUpdate(
...
// ─── Credentials ───────────────────────────────────────────────────────────
router.get("/projects/:projectId/inventory/credentials", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  const access = await guard(req, res, projectId);
  if (!access) return;
  if (access.isClient) {
...
router.post("/projects/:projectId/inventory/credentials", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId, true))) return;
...
router.post("/projects/:projectId/inventory/credentials/:id/reveal", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  const access = await guard(req, res, projectId);
  if (!access) return;
  if (access.isClient) {
...
router.delete("/projects/:projectId/inventory/credentials/:id", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId, true))) return;
...
// ─── Environments ──────────────────────────────────────────────────────────
router.get("/projects/:projectId/inventory/environments", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  const access = await guard(req, res, projectId);
  if (!access) return;
...
router.post("/projects/:projectId/inventory/environments", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId, true))) return;
...
router.patch("/projects/:projectId/inventory/environments/:id", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId, true))) return;
  const id = parseInt(req.params.id as string);
...
// ─── Devices ───────────────────────────────────────────────────────────────
router.get("/projects/:projectId/inventory/devices", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId))) return;
...
router.post("/projects/:projectId/inventory/devices", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId, true))) return;
...
// ─── Subscriptions ─────────────────────────────────────────────────────────
router.get("/projects/:projectId/inventory/subscriptions", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId))) return;
...
router.post("/projects/:projectId/inventory/subscriptions", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  if (!(await guard(req, res, projectId, true))) return;
...
// GET builds — proxy existing APK releases
router.get("/projects/:projectId/inventory/builds", requireAuth, async (req, res) => {
  const projectId = parseInt(req.params.projectId as string);
  const access = await guard(req, res, projectId);
  if (!access) return;
