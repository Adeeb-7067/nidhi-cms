...
import { formatBugRow, formatBugRows } from "../lib/bug-format";
...
// POST /api/bugs — team members can file issues; QA/admin can assign on create
router.post("/bugs", requireAuth, requireRole("developer", "tester", "super_admin"), async (req, res) => {
  const {
    projectId,
    title,
    description,
...
// PATCH /api/bugs/:id/assign — QA / admin quick assign to developer
router.patch("/bugs/:id/assign", requireAuth, requireRole("tester", "super_admin"), async (req, res) => {
  const id = Number.parseInt(req.params.id as string, 10);
