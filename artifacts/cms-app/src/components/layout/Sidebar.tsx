...
export function Sidebar() {
  ...
  return (
    <div className="flex flex-col h-screen w-64 border-r border-border bg-sidebar text-sidebar-foreground">
      <div className="p-4 border-b border-border flex items-center gap-3 h-14">
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 space-y-6">
        {filteredSections.map((section) => (
          <div key={section.label}>
            <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-sidebar-foreground/40 px-3 mb-1 mt-4">
              {section.label}
            </div>
              ...
              {section.items.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className={cn(
                    ...
                    {item.title}
                    {badgeCount > 0 && (
                      <span className="ml-auto h-5 px-1.5 rounded-full bg-destructive/15 text-destructive text-xs font-medium flex items-center">
                        {badgeCount}
                      </span>
                    )}
                  </Link>
