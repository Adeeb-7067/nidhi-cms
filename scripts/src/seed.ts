...
async function main() {
  ...
  // Seed function for generating IDs
  ...
  if (!existingProject && clientId) {
    ...
    if (david) {
      const existingMembership = await schema.projectMembersTable.findOne({ projectId: project1.id, userId: david.id });
    ...
    await schema.milestonesTable.create({ id: ms1Id, projectId: project1.id, title: "Alpha Release", plannedDate: new Date("2025-03-31"), actualDate: new Date("2025-04-05"), status: "completed" });
    
    const ms2Id = await getNextSequence("milestones");
    await schema.milestonesTable.create({ id: ms2Id, projectId: project1.id, title: "Beta Release", plannedDate: new Date("2025-06-30"), status: "pending" });
    
    const ms3Id = await getNextSequence("milestones");
    await schema.milestonesTable.create({ id: ms3Id, projectId: project1.id, title: "Production Launch", plannedDate: new Date("2025-08-30"), status: "pending" });

    console.log("Created demo project with members, logs, bugs, milestones");
  }

  // Ensure tester exists on at least one project (for existing DBs seeded before tester role)
  const david = await schema.usersTable.findOne({ email: "david@agency.com" });
