    ...
    <TableRow className="text-xs">
      ...
      <TableCell className="py-2.5">
        <div className="flex items-center gap-1 text-muted-foreground">
          ...
          <span className="whitespace-nowrap">{durationLabel(session.durationMs)}</span>
        </div>
      </TableCell>
      <TableCell className="py-2.5">
        {session.stopReason ? (
...
export default function AttendancePage() {
  ...
  const employees = allUsers.filter((u) => ["developer", "tester", "qa"].includes(u.role));
            ...
            {loadingActive ? (
              ...
              <div className="flex flex-wrap gap-2">
                {activeAll.data.map((s) => (
                  <div
                    key={s.id}
                    className="flex items-center gap-2 pl-1 pr-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800"
                  >
                    <Avatar className="h-5 w-5 shrink-0">
                      <AvatarFallback className="text-[8px] font-bold bg-emerald-500/20 text-emerald-700">
                        {initials(userMap[s.userId] ?? "?")}
                      </AvatarFallback>
                    </Avatar>
                    <span className="text-xs font-medium text-emerald-700 dark:text-emerald-400">
                      {userMap[s.userId] ?? `User #${s.userId}`}
                    </span>
