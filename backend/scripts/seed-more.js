...
async function main() {
    ...
    for (const lt of logTemplates) {
      ...
      for (let i = 0; i < Math.min(logDates.length, 20); i++) {
        ...
        await schema.dailyLogsTable.create({
          ...
          nextDayPlan: i % 3 === 0 ? "Continue with next sprint tasks" : void 0,
          blockers: i % 5 === 0 ? "Waiting for design approval" : void 0
        });
        logCount++;
      }
    }
  ...
  const bugTemplates = [
    { title: "Login page flickers on mobile Safari", severity: "high", priority: "p1", platform: "ios", status: "open", desc: "Login form briefly goes blank on iOS 17 before rendering. Happens on page load and back navigation." },
    { title: "Pagination resets to page 1 on sort", severity: "medium", priority: "p2", platform: "web", status: "in_progress", desc: "When user sorts any column while on page 3+, pagination resets to page 1 unexpectedly." },
    { title: "File upload fails for files > 5MB", severity: "critical", priority: "p1", platform: "api", status: "open", desc: "API returns 413 for files between 5-10MB despite docs claiming 25MB limit. Nginx config mismatch." },
    { title: "Dark mode toggle not persisting across tabs", severity: "low", priority: "p3", platform: "web", status: "open", desc: "Dark mode preference saved in localStorage but not synced across browser tabs using storage events." },
    { title: "Push notifications not received in background", severity: "high", priority: "p2", platform: "android", status: "in_progress", desc: "Android notifications fire correctly in foreground but silently fail when app is backgrounded." },
    { title: "Date picker breaks on Firefox", severity: "medium", priority: "p2", platform: "web", status: "open", desc: "Native date input type=date renders inconsistently on Firefox 120+. Needs custom picker component." },
    { title: "Chart tooltips overflow off-screen", severity: "low", priority: "p4", platform: "web", status: "open", desc: "Recharts tooltip goes off-screen when hovering data points near the right edge of the chart." },
    { title: "Search returns stale results after update", severity: "medium", priority: "p2", platform: "web", status: "fixed", desc: "Elasticsearch index not updated immediately after record mutation. Cache invalidation bug." },
    { title: "Memory leak in real-time dashboard", severity: "critical", priority: "p1", platform: "web", status: "in_progress", desc: "WebSocket event listeners not cleaned up on component unmount. Page memory grows to 2GB+ over time." },
    { title: "Password strength indicator missing on reset", severity: "low", priority: "p4", platform: "web", status: "open", desc: "Password strength meter shown on register page but absent from password reset flow. UX inconsistency." },
    { title: "API response times spike during peak hours", severity: "high", priority: "p1", platform: "api", status: "in_progress", desc: "P95 latency jumps from 120ms to 4.2s between 2-4pm UTC. Suspected database connection pool exhaustion." },
    { title: "CSV export includes HTML entities", severity: "medium", priority: "p3", platform: "web", status: "open", desc: "Exported CSV has &amp; instead of & in company names. Missing HTML decode step in export pipeline." },
    { title: "Session expires but user stays on dashboard", severity: "high", priority: "p2", platform: "web", status: "fixed", desc: "Expired JWT not caught on /api/dashboard endpoint. Page shows stale data instead of redirecting to login." },
    { title: "Autocomplete reveals confidential client names", severity: "critical", priority: "p1", platform: "web", status: "open", desc: "Search autocomplete shows all client names regardless of user's assigned projects. RBAC gap in suggestions API." },
    { title: "Report PDF has incorrect totals", severity: "high", priority: "p1", platform: "web", status: "open", desc: "Monthly billing report PDF shows wrong totals when line items have discounts. Off-by-one in discount calculation." },
    { title: "Calendar view missing weekends", severity: "medium", priority: "p3", platform: "web", status: "open", desc: "Project timeline calendar renders Mon-Fri only. Weekends omitted breaking multi-day task spans." },
    { title: "Bulk action deselects items on pagination", severity: "medium", priority: "p2", platform: "web", status: "in_progress", desc: "Checking all items and navigating to next page clears the selection. Needs cross-page selection state." },
    { title: "iOS app crashes on low storage", severity: "critical", priority: "p1", platform: "ios", status: "open", desc: "App crashes without error message when device has <500MB free space. No storage check or graceful fallback." },
    { title: "Webhook duplicate deliveries", severity: "high", priority: "p2", platform: "api", status: "in_progress", desc: "Webhook retry logic triggers even on 200 responses. Implementing idempotency keys needed." },
    { title: "Multi-select filter ignores OR logic", severity: "medium", priority: "p3", platform: "web", status: "open", desc: "Selecting Status=Open AND Status=In Progress should use OR logic but returns empty result set." }
  ];
  ...
  if (!existingReq && allProjects.length > 0) {
    const requests = [
      { dev: alice, type: "software_license", title: "Figma Enterprise License", urgency: "high", status: "approved", desc: "Need Figma Enterprise for the design system work across 3 active projects. Team plan insufficient.", note: "Approved. License purchased and activated." },
      { dev: bob, type: "api_access", title: "Stripe Production API Keys", urgency: "high", status: "approved", desc: "Payment gateway production keys needed to complete E-Commerce integration before UAT.", note: "Keys shared via Vault." },
      { dev: henry, type: "server_hosting", title: "AWS EC2 t3.xlarge Instance", urgency: "medium", status: "approved", desc: "Additional compute needed for staging environment performance testing under 500 concurrent users.", note: "Instance spun up. Cost center: DEV-OPS-2025." },
      { dev: carol, type: "software_license", title: "Apple Developer Program Renewal", urgency: "high", status: "pending", desc: "Annual Apple Developer account expires in 14 days. Required for iOS builds and TestFlight distribution.", note: void 0 },
      { dev: frank, type: "design_asset", title: "Premium Icon Pack \u2014 LineIcons Pro", urgency: "low", status: "pending", desc: "Consistent premium icon set needed for the CRM dashboard project. 1000+ icons, one-time $49 license.", note: void 0 },
      { dev: iris, type: "software_license", title: "Tableau Desktop License", urgency: "medium", status: "rejected", desc: "Advanced data visualization for analytics reports. Alternative free tools evaluated.", note: "Rejected \u2014 Apache Superset covers requirements at zero cost." },
      { dev: bob, type: "hardware", title: "MacBook Pro M3 Max for Load Testing", urgency: "low", status: "pending", desc: "Current dev machine struggles with Docker + k6 load testing simultaneously. Requesting upgrade.", note: void 0 },
      { dev: alice, type: "other", title: "Conference: React Summit 2025", urgency: "low", status: "approved", desc: "React Summit Amsterdam May 15-16. Valuable for upskilling on React 19, Server Components, and Suspense.", note: "Approved. Book flights and hotel in Notion." }
    ];
    let reqIdx = 0;
    ...
    for (const mp of milestonesByProject) {
      ...
      for (const m of mp.milestones) {
        ...
        await schema.milestonesTable.create({
          ...
          actualDate: m.actual ? new Date(m.actual) : void 0,
          status: m.status
        });
      }
    }
