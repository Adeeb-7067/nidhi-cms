...
export function resolveAttendanceStatus({
  ...
  const effectiveWfh = wfh || (globalWfhMode ? { id: null, global: true } : null);
  if (effectiveWfh) {
    const target = partialLeave ? workExpected : expectedMinutes;
    return {
      status: "wfh",
      compliance:
        activeMinutes >= target - threshold ? "met" : activeMinutes > 0 ? "short" : "absent",
      wfhRequestId: effectiveWfh.id ?? null,
      globalWfh: !!effectiveWfh.global,
