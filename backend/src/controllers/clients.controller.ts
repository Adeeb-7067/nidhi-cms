...
import { formatCompanyRecord } from "@/services/company-format";
import {
...
// POST /api/clients
export async function postClients(req: Request, res: Response) {
...
export async function patchClientsById(req: Request, res: Response) {
  const id = parseIdParam(req.params.id, "client id");
  const body = req.body as Record<string, unknown>;
