...
import { formatCompanyRecord, getCompanyActivity } from "@/services/company-format";
import { formatProject } from "@/services/project-format";
import { createClientPortalUser } from "@/services/client-portal";
import { assertCompanyAccess } from "@/services/access-helpers";
...
// POST /api/companies
export async function postCompanies(req: Request, res: Response) {
  const {
    companyName,
    contactPerson,
    primaryContact,
...
export async function patchCompaniesById(req: Request, res: Response) {
  const id = parseIdParam(req.params.id, "company id");
  const {
    companyName,
    contactPerson,
...
export async function getCompaniesByIdProjects(req: Request, res: Response) {
...
export async function getCompaniesByIdTickets(req: Request, res: Response) {
  const companyId = parseIdParam(req.params.id, "company id");
  await assertCompanyAccess(req, companyId);
...
export async function getCompaniesByIdActivity(req: Request, res: Response) {
  const companyId = parseIdParam(req.params.id, "company id");
  await assertCompanyAccess(req, companyId);
