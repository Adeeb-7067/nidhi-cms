...
import { CLOSED_BUG_STATUSES, resolveListStatusFilter } from "../../work/services/bug-workflow.js";
import {
  buildWorkspaceDashboard,
  buildClientHubDashboard,
} from "../../platform/services/workspace-dashboard.js";
import { buildRecentActivity, listRecentActivity } from "../../platform/services/dashboard-activity.js";
import { assertProjectAccess } from "../../access/services/access-helpers.js";
import { applyIdScope, getAccessibleProjectIds } from "../../access/services/list-scope.js";
import { forbidden, parsePagination } from "../../../utils/route-errors.js";
...
async function getAnalyticsDashboard(req, res) {
  ...
  const { countOnlineByRoles } = await import("../../monitoring/services/presence.js"); // lazy: keeps analytics bundle path light when unused
...
/** CMS-wide user activity feed (audit logs), admin Activity page. */
async function getAnalyticsActivity(req, res) {
  const { page, limit } = parsePagination(req.query);
  const actorId =
    req.query.actorId != null && req.query.actorId !== ""
      ? Number(req.query.actorId)
      : null;
  const entityType =
    req.query.entityType != null && String(req.query.entityType).trim() !== ""
