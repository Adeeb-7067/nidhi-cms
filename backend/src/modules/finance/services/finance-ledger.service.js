...
export async function computeClientLedgers(clientId = null) {
  ...
  const [invoices, salesInvoices, payments] = await Promise.all([
    FinanceInvoices.find({ clientId: { $in: clientIds }, status: { $ne: "cancelled" } }).lean(),
    SalesInvoices.find({ customerId: { $in: clientIds }, status: { $ne: "cancelled" } }).lean(),
  ...
  return clients
    .map((client) => {
      ...
      const clientSales = salesInvoices.filter((i) => i.customerId === client.id);
      ...
      for (const inv of clientSales) {
        const { total } = calcSalesInvoiceBreakdown(inv);
        rawEntries.push({
          date: inv.issueDate,
          ...
          credit: 0,
          reference: inv.number,
          referenceHref: `/sales/invoices/${inv.id}`,
        });
      }
      for (const pay of clientPayments) {
        rawEntries.push({
          ...
          referenceHref: pay.id != null ? `/finance/payments/finance/${pay.id}` : undefined,
        });
      }
...
/** Accounts-payable view: vendor bills + standalone vendor expenses − payments. */
export async function computeVendorLedgers(vendorId = null) {
  ...
  const [expenses, vendorInvoices, payments] = await Promise.all([
  ...
  return vendors
    .map((vendor) => {
      const vendorExpenses = expenses.filter(
        (e) => e.vendorId === vendor.id && e.vendorInvoiceId == null,
      );
      const bills = vendorInvoices.filter((i) => i.vendorId === vendor.id);
      const vendorPayments = payments.filter((p) => p.vendorId === vendor.id);
      const rawEntries = [
        ...bills.map((inv) => ({
          date: inv.invoiceDate,
          description: `Vendor bill ${inv.invoiceNumber}`,
          debit: 0,
          credit: inv.totalAmount,
          reference: inv.invoiceNumber,
          referenceHref: `/finance/vendors/${vendor.id}`,
        })),
        ...
        ...vendorPayments.map((p) => ({
          ...
          referenceHref: `/finance/payments/finance/${p.id}`,
        })),
      ];
      const { entries, closingBalance } = buildEntriesWithBalance(rawEntries, 0);
      return {
