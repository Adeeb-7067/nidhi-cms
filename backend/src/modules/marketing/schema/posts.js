...
const marketingPostSchema = new Schema(
  {
    ...
    /** All selected channels for this schedule (UI multi-select). `platform` remains the primary. */
    platforms: {
    ...
    /** Set when the schedule-time reminder notification was delivered. */
...
marketingPostSchema.index({ scheduledAt: 1, scheduleStatus: 1, reminderSentAt: 1 });
